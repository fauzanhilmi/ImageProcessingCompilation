...
import com.pengcit.fauzanhilmi.imageprocessingcompilation.ChainCode_Grid_KodeBelok.KodeBelok.TurnCode;
import com.pengcit.fauzanhilmi.imageprocessingcompilation.ChainCode_Grid_KodeBelok.PlatNomorDetector.NumberDetection;

import java.util.ArrayList;


public class KodeBelokActivity extends ActionBarActivity {
    Button UButton;
    TextView tv;
...
//                res += Integer.toString(ai.get(i));
            ...
            //ArrayList<Integer> kodebelok = TurnCode.processChainCode()
            res += "KodeBelok : ";
            for(int i=0; i<nd.pxTracer.areas.size(); i++) {
                ArrayList<Integer> kodebelok = TurnCode.toTurnCode(TurnCode.processChainCode(nd.pxTracer.areas.get(i).chainCode));
                res += "[";
                for(int j=0; j<kodebelok.size(); j++) {
                    res += Integer.toString(kodebelok.get(j));
                    res += ", ";
                }
                res += "]; ";
            }
